...
/// Animated duplicate-file icon — black square with diagonal hatching overlay.
/// Scales to any frame size. Use like: `DuplicateIcon().frame(width: 60, height: 60)`
struct DuplicateIcon: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel("Duplicate icon")
        .onAppear {
    ...
    private func vectorLayers(for size: CGFloat) -> some View {
        ...
        let lineSpacing   = max(16 * scale, 3)
        ...
        return ZStack {
            ...
            DiagonalLinesShape(growth: lineGrowth, spacing: lineSpacing)
